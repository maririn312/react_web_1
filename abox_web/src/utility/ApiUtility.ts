...
import { AUTH_BASIC, AUTH_BEARER, GRAND_CLIENT_CREDENTIALS, GRANT_PASSWORD, GRANT_REFRESH_TOKEN, HEADER_AUTH, HEADER_CONTENT, HEADER_FIREBASE_TOKEN, HEAD_JSON, HEAD_MULTIPART, HEAD_URLENCODED, PARAM_GRANT, PARAM_PASSWORD, PARAM_REFRESH_TOKEN, PARAM_USERNAME } from "../app/types";
import { BASIC_PASSWORD, BASIC_USERNAME, REFRESH_TOKEN_MAX_ITERATE, URL_BACKEND_AUTH } from "../app/appConst";
...
export const ApiClient = () => {
  ...
  instance.interceptors.response.use(
    async function(response) {
      const isListed = interceptorWhitelist.find(item => item === response.config.url);
...
export class ApiUtility {
  ...
  static getAccessTokenConfig = () => {
    return {
      headers: {
        [HEADER_AUTH]:`${AUTH_BASIC} ${window.btoa(`${BASIC_USERNAME}:${BASIC_PASSWORD}`)}`,
  ...
  /* ============================================================================ */
  /* ============================================================================ */
  static getUserAccessToken = async (type:string, username:string, password:string) => {
    const config = ApiUtility.getAccessTokenConfig();
  
    const params = new URLSearchParams();
    params.append(PARAM_USERNAME, `${type},${username},${username}`);
  ...
  /* ============================================================================ */
  /* ============================================================================ */
  static getRefreshToken = async (refreshToken:string, fromCache?:boolean, failedCount?:number) => {
